...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    {
    	//System.out.println("Handling chunk data");//marker
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);

        if (packetIn.doChunkLoad())
        {
            this.clientWorldController.doPreChunk(packetIn.getChunkX(), packetIn.getChunkZ(), true);
            //System.out.println("dochunkload");//marker
        }

        this.clientWorldController.invalidateBlockReceiveRegion(packetIn.getChunkX() << 4, 0, packetIn.getChunkZ() << 4, (packetIn.getChunkX() << 4) + 15, net.minecraft.world.World.heightLimit, (packetIn.getChunkZ() << 4) + 15);
        ...
        this.clientWorldController.markBlockRangeForRenderUpdate(packetIn.getChunkX() << 4, 0, packetIn.getChunkZ() << 4, (packetIn.getChunkX() << 4) + 15, net.minecraft.world.World.heightLimit, (packetIn.getChunkZ() << 4) + 15);

        if (!packetIn.doChunkLoad() || !(this.clientWorldController.provider instanceof WorldProviderSurface))
        {
            chunk.resetRelightChecks();
            //System.out.println("lightreset");//marker
        }
