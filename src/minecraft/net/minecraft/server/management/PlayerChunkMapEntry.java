...
import net.minecraft.network.play.server.SPacketChunkData.SectionMask;
...
public class PlayerChunkMapEntry
{
    ...
    private final int[] changedBlocks = new int[64];//private final short[] changedBlocks = new short[64];
    @Nullable
    private Chunk chunk;
    private int changes;
    private SectionMask changedSectionFilter;
    private int changedSectionFilterCounter;
    ...
    public void sendNearbySpecialEntities(EntityPlayerMP player)
    {
        if (this.sentToPlayers)
        {
            player.connection.sendPacket(new SPacketChunkData(this.chunk, new SectionMask(true)));
    ...
    public void blockChanged(int x, int y, int z)
    {
        if (this.sentToPlayers)
        {
            ...
            this.changedSectionFilter.setBit(y >> 4, true);;//this.changedSectionFilter |= 1 << (y >> 4);

            if (this.changes < 64)
            {
                int short1 = (int)(x << 20 | z << 16 | y);//short short1 = (short)(x << 12 | z << 8 | y);
    ...
    public void update()
    {
        if (this.sentToPlayers && this.chunk != null)
        {
            if (this.changes != 0)
            {
                if (this.changes == 1)
                {
                    int i = (this.changedBlocks[0] >> 20 & 15) + this.pos.chunkXPos * 16;
                    int j = this.changedBlocks[0] & 65535;
                    int k = (this.changedBlocks[0] >> 16 & 15) + this.pos.chunkZPos * 16;
                ...
                else
                {
                    ...
                    {
                        int i1 = (this.changedBlocks[l] >> 20 & 15) + this.pos.chunkXPos * 16;
                        int j1 = this.changedBlocks[l] & 65535;
                        int k1 = (this.changedBlocks[l] >> 16 & 15) + this.pos.chunkZPos * 16;
