...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO
{
    ...
    private final Map<ChunkPos, NBTTagCompound> extensionsToRemove = new ConcurrentHashMap();
    ...
    public Chunk loadChunk(World worldIn, int x, int z) throws IOException
    {
        ...
        NBTTagCompound extension = (NBTTagCompound) this.extensionsToRemove.get(chunkpos);
        ...
        if(extension == null) {
        	DataInputStream sideinput = RegionFileCache.getExtendedChunkInputStream(this.chunkSaveLocation, x, z);
        	if(sideinput != null) 
            	extension = this.dataFixer.process(FixTypes.CHUNK, CompressedStreamTools.read(sideinput));
        }

        return this.checkedReadChunkFromNBT(worldIn, x, z, nbttagcompound, extension);
    }
     ...
     */
    protected Chunk checkedReadChunkFromNBT(World worldIn, int x, int z, NBTTagCompound compound, NBTTagCompound extension)
    {
        ...
        else
        {
            ...
            NBTTagCompound extensiontag;
            if(extension == null) {
            	extensiontag = new NBTTagCompound();
            } else
            extensiontag = extension.getCompoundTag("Extension");
            ...
            {
                Chunk chunk = this.readChunkFromNBT(worldIn, nbttagcompound, extensiontag);
                ...
                if (!chunk.isAtLocation(x, z))
                {
                    ...
                    chunk = this.readChunkFromNBT(worldIn, nbttagcompound, extensiontag);
                }
    ...
    public void saveChunk(World worldIn, Chunk chunkIn) throws MinecraftException, IOException
    {
        ...
        try
        {
            ...
            NBTTagCompound extend = new NBTTagCompound();
            nbttagcompound.setTag("Level", nbttagcompound1);
            nbttagcompound.setInteger("DataVersion", 922);
            nbttagcompound2.setTag("Extension", extend);
            nbttagcompound2.setInteger("DataVersion", 922);
            this.writeChunkToNBT(chunkIn, worldIn, nbttagcompound1,extend);
            this.addChunkToPending(chunkIn.getChunkCoordIntPair(), nbttagcompound, nbttagcompound2);
        }
    ...
    protected void addChunkToPending(ChunkPos pos, NBTTagCompound compound,NBTTagCompound extension)
    {
        if (!this.pendingAnvilChunksCoordinates.contains(pos))
        {
            this.chunksToRemove.put(pos, compound);
            this.extensionsToRemove.put(pos, extension);
        }
    ...
    public boolean writeNextIO()
    {
        ...
        else
        {
            ...
            try
            {
                ...
                NBTTagCompound extension = (NBTTagCompound) this.extensionsToRemove.remove(chunkpos);
                ...
                if (extension != null)
                {
                    try
                    {
                        this.writeChunkExtensionData(chunkpos, extension);
                    }
                    catch (Exception exception)
                    {
                        LOGGER.error((String)"Failed to save chunk extension", (Throwable)exception);
                    }
                }
    ...
    private void writeChunkExtensionData(ChunkPos pos, NBTTagCompound compound) throws IOException
    {
        DataOutputStream dataoutputstream = RegionFileCache.getExtendedChunkOutputStream(this.chunkSaveLocation, pos.chunkXPos, pos.chunkZPos);
        CompressedStreamTools.write(compound, dataoutputstream);
        dataoutputstream.close();
    }
     ...
     */
    private void writeChunkToNBT(Chunk chunkIn, World worldIn, NBTTagCompound compound, NBTTagCompound extension)
    {
        compound.setInteger("xPos", chunkIn.xPosition);
        extension.setInteger("xPos", chunkIn.xPosition);
        compound.setInteger("zPos", chunkIn.zPosition);
        extension.setInteger("zPos", chunkIn.zPosition);
        ...
        NBTTagList listex = new NBTTagList();
        ...
        for (ExtendedBlockStorage extendedblockstorage : aextendedblockstorage)
        {
            ...
            {
            	if((extendedblockstorage.getYLocation() >> 4) < 16) {
                	NBTTagCompound nbttagcompound = new NBTTagCompound();
                	...
                	NibbleArray nibblearray = new NibbleArray();
                	NibbleArray nibblearray1 = extendedblockstorage.getData().getDataForNBT(abyte, nibblearray);
                	extensiontag.setByteArray("Blocks", abyte);
                	extensiontag.setByteArray("Data", nibblearray.getData());
                
                	if (nibblearray1 != null)
                	{
                    	extensiontag.setByteArray("Add", nibblearray1.getData());
                	}
                
                	extensiontag.setByteArray("BlockLight", extendedblockstorage.getBlocklightArray().getData());
                
                	if (flag)
                	{
                	extensiontag.setByteArray("SkyLight", extendedblockstorage.getSkylightArray().getData());
                	}
                	else
                	{
                    	extensiontag.setByteArray("SkyLight", new byte[extendedblockstorage.getBlocklightArray().getData().length]);
                	}
                
                	listex.appendTag(extensiontag);
            	}
            }
        }

        compound.setTag("Sections", nbttaglist);
        extension.setTag("Sections", listex);
     ...
     */
    private Chunk readChunkFromNBT(World worldIn, NBTTagCompound compound, NBTTagCompound extension)
    {
        ...
        int k = 16;
        ExtendedBlockStorage[] aextendedblockstorage = new ExtendedBlockStorage[4096];
        boolean flag = worldIn.provider.func_191066_m();
        ...
        if(extension != null) {
        	NBTTagList list2 = extension.getTagList("Sections", 10);
        	for(int l0 = 0; l0 < list2.tagCount() ; ++l0 ) {
        		NBTTagCompound nbttagcompound = list2.getCompoundTagAt(l0);
        		int i1 = nbttagcompound.getInteger("Y");
        		ExtendedBlockStorage extendedblockstorage = new ExtendedBlockStorage(i1 << 4, flag);
        		byte[] abyte = nbttagcompound.getByteArray("Blocks");
        		NibbleArray nibblearray = new NibbleArray(nbttagcompound.getByteArray("Data"));
        		NibbleArray nibblearray1 = nbttagcompound.hasKey("Add", 7) ? new NibbleArray(nbttagcompound.getByteArray("Add")) : null;
...
//        NBTTagList ents = extension.getTagList("Entities", 10);
//        for(int n = 0; n < ents.tagCount() ;n++) {
//        	nbttaglist1.appendTag(ents.getCompoundTagAt(n));
//        }
