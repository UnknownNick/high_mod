...
public class RegionExtendFile
{
    ...
    public RegionExtendFile(File fileNameIn)
    {
        ...
        try
        {
            ...
            {
                this.dataFile.write(EMPTY_SECTOR);// 1/2 offset
                this.dataFile.write(EMPTY_SECTOR);// 2. 1/2 offset
                this.dataFile.write(EMPTY_SECTOR);//timestamps
                this.sizeDelta += 3*4096;
            }

            if ((this.dataFile.length() & 4095L) != 0L) //padding
            {
            ...
            this.sectorFree.set(2, Boolean.valueOf(false));
            ...
            {
            	/**offset read*/
                long k = this.dataFile.readLong();
                this.offsets[j1] = k;

                /*offset format bytes 0,1 - padding
                 * 				bytes 2,3,4,5 - first sector
                 *              bytes 7,8 - sector count
                 */
                if (k != 0 && (k >> 16) + (k & 65535) <= this.sectorFree.size())
                {
                    for (int l = 0; l < (k & 65535); ++l)
                    {
                        this.sectorFree.set((int)((k >> 16) & 2147483647L) + l, Boolean.valueOf(false));
                    }
                }
            }
    ...
    public synchronized DataInputStream getChunkDataInputStream(int x, int z)
    {
        ...
        else
        {
            try
            {
                long i = this.getOffset(x, z);
                //chunk not present in file
                if (i == 0L)
                {
                    return null;
                }
                ...
                {
                    long j = (int)((i >> 16) & 2147483647L); //first sector
                    long k = i & 65535;//sector count

                    if (j + k > this.sectorFree.size()) //chunk does not fit
                    {
                        return null;
                    }
                    else
                    {
                        ...
                        int l = this.dataFile.readInt(); //lenght in bytes

                        if (l > 4096 * k) //lenght provided by chunk data is greater than in offset > no fit
                        {
                            return null;
                        }
                        else if (l <= 0)
                        ...
                        {
                            byte b0 = this.dataFile.readByte(); //compress scheme
    ...
    {
        return this.outOfBounds(x, z) ? null : new DataOutputStream(new BufferedOutputStream(new DeflaterOutputStream(new RegionExtendFile.ChunkBuffer(x, z))));
    }
    ...
    protected synchronized void write(int x, int z, byte[] data, int length)
    {
        ...
        {
            long i = this.getOffset(x, z);
            int j = (int)i >> 16;//1st sector
            int k = (int)i & 65535;//sector count
            ...
            if (j != 0 && k == l)//chunk size & position same
            {
                this.write(j, data, length); 
            }
            else
            {
                for (int i1 = 0; i1 < k; ++i1)//free old sectors
                {
                    this.sectorFree.set(j + i1, Boolean.valueOf(true));
                }

                int l1 = this.sectorFree.indexOf(Boolean.valueOf(true));//first free sector
                int j1 = 0;
     ...
     */
    private void setOffset(int x, int z, long offset) throws IOException
    {
        this.offsets[x + z * 32] = offset;
        this.dataFile.seek((long)((x + z * 32) * 8));
        this.dataFile.writeLong(offset);
    }
    ...
    private void setChunkTimestamp(int x, int z, int timestamp) throws IOException
    {
        ...
        this.dataFile.seek((long)(8192 + (x + z * 32) * 4));
    ...
    class ChunkBuffer extends ByteArrayOutputStream
    {
        ...
        public void close() throws IOException
        {
            RegionExtendFile.this.write(this.chunkX, this.chunkZ, this.buf, this.count);
        }
    }
}
