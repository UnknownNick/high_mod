...
public abstract class WorldGenHugeTrees extends WorldGenAbstractTree
{
    ...
    private boolean isSpaceAt(World worldIn, BlockPos leavesPos, int height)
    {
        ...
        if (leavesPos.getY() >= 1 && leavesPos.getY() + height + 1 <= net.minecraft.world.World.heightLimit)
        {
            for (int i = 0; i <= 1 + height; ++i)
            {
                ...
                for (int k = -j; k <= j && flag; ++k)
                {
                    ...
                    {
                        if (leavesPos.getY() + i < 0 || leavesPos.getY() + i >= net.minecraft.world.World.heightLimit || !this.canGrowInto(worldIn.getBlockState(leavesPos.add(k, i, l)).getBlock()))
                        {
                            flag = false;
                        }
                    }
