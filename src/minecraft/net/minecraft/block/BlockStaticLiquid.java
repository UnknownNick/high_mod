...
public class BlockStaticLiquid extends BlockLiquid
{
            ...
            if (worldIn.getGameRules().getBoolean("doFireTick"))
            {
                ...
                if (i > 0)
                {
                    ...
                    for (int j = 0; j < i; ++j)
                    {
                        ...
                        if (blockpos.getY() >= 0 && blockpos.getY() < net.minecraft.world.World.heightLimit && !worldIn.isBlockLoaded(blockpos))
                        {
                            return;
                        }
                ...
                else
                {
                    for (int k = 0; k < 3; ++k)
                    {
                        ...
                        if (blockpos1.getY() >= 0 && blockpos1.getY() < net.minecraft.world.World.heightLimit && !worldIn.isBlockLoaded(blockpos1))
                        {
                            return;
                        }
    ...
    {
        return pos.getY() >= 0 && pos.getY() < net.minecraft.world.World.heightLimit && !worldIn.isBlockLoaded(pos) ? false : worldIn.getBlockState(pos).getMaterial().getCanBurn();
    }
}
